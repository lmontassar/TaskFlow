      ...
      try {
        ...
        if (response.ok) {
          ...
          switch (response.status) {
            case 404: {
              setErrorMessage(t("OTP.errors.verify.status_404"));
              break;
            }
            case 403: {
              setErrorMessage(t("OTP.errors.verify.status_403"));
              break;
            }
            case 429: {
              setErrorMessage(t("OTP.errors.verify.status_429"));
              break;
            }
            case 401: {
              setErrorMessage(t("OTP.errors.verify.status_401"));
              break;
            }
          }
          setOtp("");
  ...
  const resendCode = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        switch (res.status) {
          case 403: {
            setErrorMessage(t("OTP.errors.send_code.status_403"));
            break;
          }
          case 500: {
            setErrorMessage(t("OTP.errors.send_code.status_500"));
            break;
          }
          case 429: {
            setErrorMessage(t("OTP.errors.send_code.status_429"));
            break;
          }
        }
      }

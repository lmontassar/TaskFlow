...
export default function SpecificRessourcesTask({
  task,
  setTask,
  canEdit,
}: Props) {
  ...
  const {
    AddNecessaryRessource,
    EdiNecessaryRessource,
    deleteNecessaryRessource,
  } = useNessasaryRess();

  // Resource types and their categories
  // const RESOURCE_TYPES = {
  //   ENERGETIC: t("task.global_resource.energetic", "Energetic"),
  //   TEMPORARY: t("task.global_resource.temporary", "Temporary"),
  ...
  const getCategoriesForType = (type: any) => {
    ...
    return typeData && typeData.categories.length > 0
      ? typeData.categories
      : [];
  };
  ...
  const handleDeleteResource = async (resourceId: any) => {
    ...
    if (res == false) return;
    setResources(
      resources.filter((resource: any) => resource.id !== necessaryToDelete)
    );
    ...
    if (isEditing) {
      ...
      setResources(
        resources.map((resource: any) =>
          resource.id === currentResource.id ? newResource : resource
        )
      );
    } else {
  ...
  return (
    <Card className="mb-4">
      {necessaryToDelete && (
        <ConfirmAlert
          key="delete-task"
          confirmDelete={necessaryToDelete}
          setConfirmDelete={setNecessaryToDelete}
          FunctionToDO={handleDeleteResource}
          Title={t(
            "task.global_resource.delete_title",
            "Are you sure you want to delete this?"
          )}
          Description={t(
            "task.global_resource.delete_description",
            "This necessary resource will be deleted permanently."
          )}

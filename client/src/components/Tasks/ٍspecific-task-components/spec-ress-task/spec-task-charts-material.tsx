...
export default function TaskResourceStatisticMaterial({
  task,
  canEdit,
}: Props) {
        ...
        <div className="text-xs space-y-1">
          <p>
            <span className="font-medium">
              {t("task.chart_material.type", "Type")}:
            </span>{" "}
            {resource.ress.type}
          </p>
          <p>
            <span className="font-medium">
              {t("task.chart_material.category", "Category")}:
            </span>{" "}
            {resource.ress.categorie || "N/A"}
          </p>
          <p>
            <span className="font-medium">
              {t("task.chart_material.quantity", "Quantity")}:
            </span>{" "}
            {resource.qte}
          </p>
          {resource.ress.unitMeasure && (
            <p>
              <span className="font-medium">
                {t("task.chart_material.unit", "Unit")}:
              </span>{" "}
              {resource.ress.unitMeasure}
            </p>
          )}
          <p>
            <span className="font-medium">
              {t("task.chart_material.cost", "Cost")}:
            </span>{" "}
            {resource.ress.coutUnitaire}
          </p>
          <p>
            <span className="font-medium">
              {t("task.chart_material.start", "Start")}:
            </span>{" "}
            {format(task.start, "MMM dd, yyyy HH:mm")}
          </p>
          <p>
            <span className="font-medium">
              {t("task.chart_material.end", "End")}:
            </span>{" "}
            {format(task.end, "MMM dd, yyyy HH:mm")}
          </p>
          {resource.ress.notes && (
            <p>
              <span className="font-medium">
                {t("task.chart_material.notes", "Notes")}:
              </span>{" "}
              {resource.ress.notes}
            </p>
          )}
  ...
  useEffect(() => {
    ...
    const valid = task.ressources.filter(
      (r) =>
        r.dateDebut &&
        r.dateFin &&
        isValid(parseISO(r.dateDebut)) &&
        isValid(parseISO(r.dateFin))
    );
  ...
  useLayoutEffect(() => {
    ...
    const showOnlyImportantDates = () => {
      const dayCells =
        container.querySelectorAll<HTMLDivElement>("[data-date]");
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>
            {t(
              "task.chart_material.material_resources_timeline",
              "Material Resources Timeline"
            )}
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-center text-muted-foreground py-8">
            {t(
              "task.chart_material.no_resources",
              "No resources with scheduled dates available. Please assign start and end dates to your resources."
            )}
          </p>
        </CardContent>
      </Card>
        ...
        <div className="flex items-center justify-between">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent className="max-w-sm">
                  <p>
                    {t(
                      "task.chart_material.tooltip",
                      "Hover over resource bars to see detailed information"
                    )}
                  </p>
                </TooltipContent>
              </Tooltip>
          ...
          >
            <SelectTrigger className="w-[120px]">
              <SelectValue
                placeholder={t("task.chart_material.view_mode", "View Mode")}
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={ViewMode.Day}>
                {t("task.chart_material.day", "Day")}
              </SelectItem>
              <SelectItem value={ViewMode.Week}>
                {t("task.chart_material.week", "Week")}
              </SelectItem>
              <SelectItem value={ViewMode.Month}>
                {t("task.chart_material.month", "Month")}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>

...
export function OptimizeDialog({ isOpen, onClose }: OptimizeDialogProps) {
  ...
  if (showChanges) {
    return <OptimizationChanges isOpen={true} onClose={handleCancel} onSave={handleSaveChanges} />
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleCancel()}>
      {showConfirmation ? (
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-center text-xl font-bold">Optimization Loading</DialogTitle>
            <DialogDescription className="text-center">Please wait, this may take a few minutes...</DialogDescription>
          </DialogHeader>
          <div className="py-6">
            <div className="space-y-6">
              {steps.map((step, index) => (
                <div key={index} className="flex items-center">
                  ...
                  <div className={`ml-4 ${index <= currentStep ? "text-foreground" : "text-muted-foreground"}`}>
                    {step}
                  </div>
                </div>
              ))}

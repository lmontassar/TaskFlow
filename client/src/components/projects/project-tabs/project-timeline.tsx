...
export function ProjectTimeline({ project }: any) {
  ...
  function CloudStatusIcon({ status }: any) {
    ...
    switch (status) {
      ...
      default:
        Icon = Cloud;
        label = t("project.tabs.timeline.synced");
    }
    return (
      <div
        className={`
    ...
    import("dhtmlx-gantt").then(({ gantt }) => {
      ...
      console.error = (...args) => {
        if (
          ...
          args[0].includes("Invalid link type")
        ) {
          return;
        }
      ...
      import("dhtmlx-gantt").then(({ gantt }) => {
        try {
          ...
          gantt.attachEvent("onAfterLinkDelete", async (id, link) => {
            if (dep == true) return;
            if (link.type == "1") {
              const res: any = await DeleteParallelTask(
                link.source,
                link.target,
                false
              );
              ...
              const res: any = await DeletePrecTask(
                link.target,
                link.source,
                false
              );
  ...
  const configureGantt = (gantt: any) => {
    ...
    gantt.config.columns = [
      {
        name: "text",
        label: t("project.tabs.timeline.task_name"),
        tree: true,
        width: 200,
      },
      {
        name: "start_date",
        label: t("project.tabs.timeline.start_date"),
        align: "center",
        width: 100,
        template: (task: any) => {
      ...
      {
        name: "end_date",
        label: t("project.tabs.timeline.end_date"),
        align: "center",
        width: 100,
        template: (task: any) => {
    ...
    gantt.templates.tooltip_text = (start: Date, end: Date, task: any) => {
      ...
      const taskLinks = allLinks.filter(
        (link) => link.source === task.id || link.target === task.id
      );
      ...
      if (taskLinks && taskLinks.length > 0) {
        ...
        taskLinks.forEach((link) => {
          ...
          if (link.source === task.id) {
            ...
            tooltip += `<br/>- ${t("Predecessor")}: ${sourceTask.text} (${t(
              linkType
            )})`;
          }
        });
  ...
  const formatTasksForGantt = (taskList: any[]) => {
    ...
    const tasks = taskList.map((task) => {
      ...
      const formattedTask = {
        ...
        text:
          typeof task.nomTache === "string" && task.nomTache.trim() !== ""
            ? task.nomTache
            : "Unnamed Task",
      ...
      if (task.paralleles?.length) {
        for (const par of task.paralleles) {
          if (
            links.some(
              (l: any) =>
                l.id === `ss-${par.id}-${task.id}` ||
                l.id === `ss-${task.id}-${par.id}`
            )
          )
            continue;
          else
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("project.tabs.timeline.title")}</CardTitle>
      </CardHeader>
      <CardContent>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium">
                  {t("project.tabs.timeline.views.label")}:
                </span>
                <Select
                  value={viewMode}
                  onValueChange={(value) =>
                    handleViewModeChange(value as "day" | "week" | "month")
                  }
                >
                  <SelectTrigger className="w-[120px]">
                    <SelectValue
                      placeholder={t("project.tabs.timeline.views.select")}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="day">
                      {t("project.tabs.timeline.views.day")}
                    </SelectItem>
                    <SelectItem value="week">
                      {t("project.tabs.timeline.views.week")}
                    </SelectItem>
                    <SelectItem value="month">
                      {t("project.tabs.timeline.views.month")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="mb-6 space-y-4">
                <div className="flex items-center gap-2">
                  <h2 className="text-xl font-semibold underline">
                    <Link to={`/task/${selectedTask.id}`}>
                      {selectedTask.nomTache}
                    </Link>
                  </h2>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-2">
                ...
                {selectedTask.dateFin && (
                  <div>
                    ...
                    <span className="text-sm">
                      {formatDate(selectedTask.dateFin)}
                    </span>
                  </div>
                )}
              </div>

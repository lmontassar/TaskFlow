...
export default function TaskPage({ params }: { params: { taskId: string } }) {
  ...
  const { tasks, handleFindAllTasks, handleUpdateTask, handleDeleteTask } = useTasks()
    ...
    <div className="container mx-auto py-8">
      ...
      {isEditing ? (
        <Card>
          ...
          <CardContent>
            <TaskEditForm task={task} onSave={handleUpdate} onCancel={() => setIsEditing(false)} />
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="text-muted-foreground">{formatDate(task.dateCreation)}</p>
                  </div>
                  {task.dateDebut && (
                    <div>
                      <h4 className="font-medium mb-1">Started</h4>
                      <p className="text-muted-foreground">{formatDate(task.dateDebut)}</p>
                    </div>
                  )}
                  {task.dateFinEstime && (
                    <div>
                      <h4 className="font-medium mb-1">Due Date</h4>
                      <p className="text-muted-foreground">{formatDate(task.dateFinEstime)}</p>
                    </div>
                  )}
                  {task.dataFin && (
                    <div>
                      <h4 className="font-medium mb-1">Completed</h4>
                      <p className="text-muted-foreground">{formatDate(task.dataFin)}</p>
                    </div>
                  )}
                </div>
              </CardContent>
                        ...
                        {task.assignee.map((assignee) => (
                          <div key={assignee.id} className="flex items-center gap-3">
                            <Avatar className="h-10 w-10">
                              <AvatarImage
                                src={assignee.avatar}
                                alt={assignee.name || `${assignee.prenom} ${assignee.nom}`}
                              />
                              <AvatarFallback>{assignee.initials}</AvatarFallback>
                            </Avatar>
                            <div>
                              <p className="font-medium">{assignee.name || `${assignee.prenom} ${assignee.nom}`}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                <Card>
                  <CardContent className="pt-6">
                    {task.attachments && task.attachments.length > 0 ? (
                      ...
                      <div className="text-center py-6 text-muted-foreground">No attachments yet</div>
                    )}
                  </CardContent>
                </Card>
              </TabsContent>
                  ...
                  {task.qualite !== 0 && task.qualite !== undefined ? (
                    <div className="flex items-center mt-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          className={`h-5 w-5 ${i < (task.qualite || 0) ? "text-blue-500 fill-blue-500" : "text-muted-foreground"}`}
                        />
                      ))}
                    </div>
                  ) : (
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>{typeof task.duree === "number" ? formatDurationReact(task.duree) : task.duree}</span>
                      </div>
                    </div>
                  </>
                )}
                ...
                {task.marge && (
                  <>
                    ...
                    <div>
                      ...
                      <p>{typeof task.marge === "number" ? formatDurationReact(task.marge) : task.marge}</p>
                    </div>
                  </>
                )}
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={task.rapporteur.avatar} alt={task.rapporteur.prenom} />
                          <AvatarFallback>{task.rapporteur.nom[0].toUpperCase()}</AvatarFallback>
                        </Avatar>
                        <span>
                          {task.rapporteur.prenom.charAt(0).toUpperCase() + task.rapporteur.prenom.slice(1)}{" "}
                          {task.rapporteur.nom.charAt(0).toUpperCase() + task.rapporteur.nom.slice(1)}
                        </span>
                      </div>
                    </div>
                  </>
